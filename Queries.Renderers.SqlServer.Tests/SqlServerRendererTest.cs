    ...
    public class SqlServerRendererTest
    {
        ...
        public static IEnumerable<object[]> UpdateTestCases
        {
            get
            {
                ...
                yield return new object[]
                {
                     Update("members").Set("firstname".Field().EqualTo(null)).Where(new WhereClause("firstname".Field(), ClauseOperator.EqualTo, "")), false,
        ...
        private static void IsQueryOk(IQuery query, bool prettyPrint, string expectedString) => Assert.Equal(expectedString, query.ForSqlServer(prettyPrint));
