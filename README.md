...
[![GitHub Main branch Status](https://img.shields.io/github/actions/workflow/status/candoumbe/queries/delivery.yml?branch=main&label=main)](https://github.com/candoumbe/Queries/actions/workflows/delivery.yml)
[![GitHub Develop branch Status](https://img.shields.io/github/actions/workflow/status/candoumbe/queries/integration.yml?branch=develop&label=develop)](https://github.com/candoumbe/Queries/actions/workflows/integration.yml)
...
[![Queries.Core](https://img.shields.io/nuget/vpre/queries.core?label=Queries.Core)](https://nuget.org/packages/queries.core)
...
<!-- TOC -->
- [Why?](#lnk-why)
- [No more tightly coupled SQL string](#lnk-coupling)
- [SQL queries secured by default](#sql-queries-secured-by-default)
- [Building statements](#building-statements)
    - [<a href="#" id='section-columns'>Columns</a>](#lnk-columns)
    ...
    - [Criteria](#criteria)
        ...
        - [Having](#having)
    - [Functions](#functions)
- [Rendering statements](#rendering-statements)
    - [SQL Server Query Renderer](#-sql-server-query-renderer)
    - [MySQL Query Renderer](#mysql-query-renderer)
    - [Sqlite Query Renderer](#sqlite-query-renderer)
...
The idea of this project came to me when I dealt with Entity Framework 6.x Code First for a project I was working on as an
Architect.

We used Migrations to make changes to our database, and sometimes we needed to write plain SQL statements as part of
Entity Framework migrations.
For this, the EF 6.x <code>Sql(...)</code> command allows to add additional SQL statements that will be executed
alongside the migrations.
But I wasn't happy with that approach as the written SQL was tightly coupled to the database engine those migrations
were run against.
I wanted something more dynamic allowing to code SQL once in the migrations and be sure that it will run smoothly if we
switch from SQL Server to PostgreSQL / MySQL / ... .
...
But when using SQL string throughout one's codebase, it can quickly become a tedious task to secure every SQL
query.<br />
Sure there's the ADO.NET library that provides various classes to create parameterized queries but this adds more and
...
- [FieldColumn][class-columns-field] contains a column name.
- [LiteralColumn][class-columns-literal]
  Uses the following classes whenever you want to write a "raw" data in a query

- [BooleanColumn][class-columns-boolean]: a column that can contain a boolean value.
...
- [DateTimeColumn][class-columns-datetime] : a [IColumn][class-columns-icolumn] implementation that can contains a
  `date`/`time`/`datetime` value.<br />
  Use this class to output a `DateTime`/`DateTimeOffset` value.

- [StringColumn][class-columns-string] : an [IColumn][class-columns-icolumn] implementation that contains "string"
  values
...
You can optionally specify a format to use when rendering the variable with the `Format(string format)` extension
method.
...
Create a [`SelectQuery`][class-select-query] instance either by using the builder or the fluent syntax to build (drum
rolling ...) a [SELECT query](https://www.w3schools.com/sql/sql_select.asp)
...
Create a [`UpdateQuery`][class-update-query] instance either by using the builder or the fluent syntax to build (drum
rolling ...) an [UPDATE](https://www.w3schools.com/sql/sql_update.asp) statement
...
### [Delete][class-delete-query]

Create a [`DeleteQuery`][class-delete-query] instance either by using the builder or the fluent syntax to build (drum
rolling ...) a [DELETE](https://www.w3schools.com/sql/sql_delete.asp) statement
...
### [Insert][class-insert-query]

Create a [`InsertIntoQuery`][class-insert-query] instance either by using the builder or the fluent syntax to build (
drum rolling ...) an [INSERT INTO](https://www.w3schools.com/sql/sql_insert.asp) statement
...
- [CompositeWhereClause][class-complex-where-clause] : combine several [IWhereClause][class-iwhere-clause] instances
  together.

### Having

- [HavingClause][class-having-clause] : a criterion that will be applied to only one field of an [IQuery][class-iquery]
- [CompositeHavingClause][class-complex-having-clause] : allows you to combine
  several [IHavingClause][class-ihaving-clause] instances.
...
- `PrettyPrint`: a hint for renderers to try to generate a "beautiful" string when rendering SQL.
- `Parametrization` : a hint for renderers on how to handle all variables a [IQuery] might embed.
  This is useful when variables' declaration has already been taken care of (
  see [CollectVariableDeclaration](#collect-variables))

# How to install ?

1. Run `dotnet add package Queries.Core`  command to get the latest version of
   the [Queries.Core](https://www.nuget.org/packages/Queries.Core/)
...
|------------|---------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| SQL Server | :2022+  | [![Queries.Renderers.SqlServer](https://img.shields.io/nuget/vpre/queries.renderers.sqlserver)](https://nuget.org/packages/queries.renderers.sqlserver) |
| Postgres   | 16+     | [![Queries.Renderers.Postgres](https://img.shields.io/nuget/vpre/queries.renderers.postgres)](https://nuget.org/packages/queries.renderers.postgres)    |
| SQLite     |         | [![Queries.Renderers.Sqlite](https://img.shields.io/nuget/vpre/queries.renderers.sqlite)](https://nuget.org/packages/queries.renderers.sqlite)          |
| Neo4J      |         | [![Queries.Renderers.Neo4J](https://img.shields.io/nuget/vpre/queries.renderers.neo4j)](https://nuget.org/packages/queries.renderers.neo4j)             |
| MySql      |         | [![Queries.Renderers.MySql](https://img.shields.io/nuget/vpre/queries.renderers.mysql)](https://nuget.org/packages/queries.renderers.mysql)             |
