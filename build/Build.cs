...
[GitHubActions("nightly-manual", GitHubActionsImage.UbuntuLatest,
    ...
    InvokedTargets = [nameof(ICompile.Compile), nameof(IMutationTest.MutationTests), nameof(IPushNugetPackages.Pack)],
    CacheKeyFiles =
    [
        "src/**/*.csproj",
            "test/**/*.csproj",
        ...
        get
        {
            ...
            return projects
                .Select(projectName => new MutationProjectConfiguration(sourceProject: Solution.AllProjects.Single(csproj => string.Equals(csproj.Name, projectName, StringComparison.InvariantCultureIgnoreCase)),
                                                                        testProjects: Solution.AllProjects.Where(csproj => csproj.Name.EndsWith(".Tests", StringComparison.InvariantCultureIgnoreCase)),
